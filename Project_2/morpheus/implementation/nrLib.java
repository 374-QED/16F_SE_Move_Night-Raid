...
public class nrLib {
	...
	public static List<String> findTime(String course, String date) throws SQLException
	{
		...
		ResultSet student_id;
		if(date.charAt(0) == 'M')
			student_id = test.readDatabase("select distinct Banner_id from classes where Subject_Code = '"+parse[0]+"' and Course_Number ='"+parse[1]+"' and Term_Code = '"+semester+"' and Monday_Ind1 = 'M' and Wednesday_Ind1 = 'W'and Friday_Ind1 = 'F'");
		else
			student_id = test.readDatabase("select distinct Banner_id from classes where Subject_Code = '"+parse[0]+"' and Course_Number ='"+parse[1]+"' and Term_Code = '"+semester+"' and Tuesday_Ind1 = 'T' and Thursday_Ind1 = 'R'");
		...
		for(int x = 0; x < student.size(); x++)
		{
			ResultSet class_t;
			if(date.charAt(0) == 'M')
				class_t = test.readDatabase("select distinct Begin_Time from classes where Banner_id = '"+student.get(x)+"' and Term_Code = '"+semester+"' and Monday_Ind1 = 'M' and Wednesday_Ind1 = 'W'and Friday_Ind1 = 'F'");
			else
				class_t = test.readDatabase("select distinct Begin_Time from classes where Banner_id = '"+student.get(x)+ "' and Term_Code = '"+semester+"' and Tuesday_Ind1 = 'T' and Thursday_Ind1 = 'R'");
